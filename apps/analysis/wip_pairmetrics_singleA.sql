...
-- Create Keyword Groupings for relatedKeywords (B)
...
-- Treat all similarKeywords (A) as a Single Cluster
CREATE TEMP TABLE "TempSingleClusterA" AS
SELECT 
    'A_cluster' AS "singleClusterId",  -- Assign a single cluster ID to all similarKeywords
...
-- Calculate Support for B (relatedKeywords grouped by keywordGroupId)
...
-- Calculate Support for A AND B (single cluster for similarKeywords and relatedKeywords grouped by keywordGroupId)
CREATE TEMP TABLE "TempSupportSingleAB" AS
SELECT
    'A_cluster' AS "singleClusterId",  -- Use single cluster identifier
    kg."keywordGroupId",
...
-- Step 6. AB Rankings ---
-- Compute Confidence and Lift for the Single Cluster
...
FROM "TempSupportSingleAB" ab
INNER JOIN "TempSingleClusterA" a ON ab."singleClusterId" = a."singleClusterId"
INNER JOIN "TempSupportB" b ON ab."keywordGroupId" = b."keywordGroupId";

-- Create Final Results with Readable Descriptions for Single Cluster
CREATE TEMP TABLE "TempFinalResults" AS
SELECT 
    cl."singleClusterId" AS "similarKeywordCluster",
    'All Similar Keywords' AS "similarKeywordName",  -- Represent all similar keywords collectively
    cl."keywordGroupId",
...
GROUP BY cl."singleClusterId", cl."keywordGroupId", cl."supportAAndB", cl."supportA", cl."supportB", cl."confidence", cl."lift";
