import { openai } from ".."
import {
	TokenizedCaseSearchPrompt,
	TokenizedCaseSearchPromptJSONSchema,
	TokenizedCaseSearchPromptSchema,
} from "../../keywords/schema/tokenizedCaseSearchPromptSchema"

const MODEL = "gpt-4o-2024-08-06"
const SYSTEM_MESSAGE =
	"You are medical keyword identifier for a patient case search prompt"

const USER_MESSAGE = `
    You are being given a user prompt in which they have a medical case which they need to find related data with. The database which will query off of provided prompt is a collection of medical cases with patient notes.
	I would like you to first think of the notes as three components: context (context for the medical case, before this instance of care), procedures (what was attempted during care), and result of procedures.
	Then for each group, I would like you to identify medical keywords the user will want to search other cases by, give it a semantic name which can have multiple tokens separated by a space but should only represent one "thing" without quantitative or qualitative description, and categorize them into the given categories:
...
export const tokenizeCaseSearchPrompt = async (
	userPrompt: string,
): Promise<TokenizedCaseSearchPrompt> => {
	const response = await openai.chat.completions.create({
		...
		messages: [
			...
			{
				role: "user",
				content: `${USER_MESSAGE} : ${userPrompt}`,
			},
		],
		response_format: {
			type: "json_schema",
			json_schema: {
				schema: TokenizedCaseSearchPromptJSONSchema,
				name: "TokenizedCaseSearchPromptJSONSchema",
			},
		},
	})
	...
	if (result) {
		const functionResult = TokenizedCaseSearchPromptSchema.parse(
			JSON.parse(result),
		)
